...
#python start.py ordenado archivo_prob archivo_blocks archivo_sols restringido parada x_binario opcion_v
...
if ordenado == '1':
  ...
  model,info,instancia = reader(archivo_prob,archivo_blocks)
  model.setParam('NumericFocus',3)
  y0_array,x0_array,times_k,q_array,v_array = original_solver(model, instancia, info, option = 'pwl', flag_full = flag_full, x_binary = x_binary, previous = None, parada = parada, voption = voption)
  y0_array = pd.DataFrame(data = y0_array)
  sol_to_OMP(y0_array,archivo_sols)